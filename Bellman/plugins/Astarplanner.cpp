...
vector<int> AStarPlanner::runAStarOnGrid(int startGridSquare, int goalGridSquare)
{
  ...
  // 初始化成本数组
    ...
    for (uint i = 0; i < mapSize; i++)
        g_score[i] = infinity;
    g_score[startGridSquare] = 0; // 开始点的成本为0

    // 进行松弛操作
    for (int i = 0; i < mapSize - 1; i++) {
        for (int u = 0; u < mapSize; u++) { // 遍历每个节点
            vector<int> neighbors = findFreeNeighborGridSquare(u);
            for (int v : neighbors) { // 遍历每个邻居节点
                float weight = getMoveCost(u, v);
                if (g_score[u] + weight < g_score[v]) {
                    g_score[v] = g_score[u] + weight; // 松弛操作
                }
            }
        }
    }
    ...
    // 构建路径（如果存在）
    vector<int> bestPath = constructPath(startGridSquare, goalGridSquare, g_score);
    return bestPath;
}
