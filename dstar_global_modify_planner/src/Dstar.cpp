...
void Dstar::updateVertex(state u) {
  ...
  for (i=s.begin();i != s.end(); i++) {
  	cost_temp  = getG(*i) + cost(u,*i);
  	if (cost_temp < min_cost) min_cost = cost_temp;
    	}
  if (!close(getRHS(u),min_cost)) {
  	setRHS(u,min_cost);
  }
...
bool Dstar::replan() {
  ...
  path.clear();
    ...
    // Prepare to parallelize successor processing
    std::vector<std::thread> threads;
    std::mutex mutex;
    
    // Function to process each successor in parallel
    auto process_successor = [&](state succ) {
        double val = cost(cur, succ) + getG(succ);
        double val2 = trueDist(succ, s_goal) + trueDist(s_start, succ);

        std::lock_guard<std::mutex> lock(mutex);  // Ensure that access to shared variables is safe
        if (close(val, cmin)) {
            if (tmin > val2) {
                tmin = val2;
                cmin = val;
    ...
    // Launch a thread for each successor
    for (state& succ : n) {
        threads.emplace_back(std::thread(process_successor, succ));
    }
